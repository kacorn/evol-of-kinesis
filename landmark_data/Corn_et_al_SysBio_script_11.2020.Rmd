...
#you can highlight a single strike at a time by just adjusting the dataset you color. like make the alpha on everything else 0.2 and make your strike of interest red or something like that.
```

```{r warp grids of pc extremes}
...
```{r procD.pgls for phylogenetic anova}
require(geomorph)
...
#set up df
disp_test_func_out <- data.frame(rep(NA,7))
disp_test_func_out$trait <- rep(NA,7)
disp_test_func_out$var_biters <- rep(NA,7)
disp_test_func_out$var_suction <- rep(NA,7)
disp_test_func_out$props <- rep(NA,7)
disp_test_func_out$p.dfc <- rep(NA,7)
...
for(iter in 1:7){
  ...
  disp_test_func_out$trait[iter] <- var_names_vec[iter]
  disp_test_func_out$var_biters[iter] <- out$Procrustes.var[[1]]
  disp_test_func_out$var_suction[iter] <- out$Procrustes.var[[2]]
  disp_test_func_out$p.dfc[iter] <- out$PV.dist.Pval[1,2]
  disp_test_func_out$props[iter] <- out$Procrustes.var[[2]]/out$Procrustes.var[[1]]
  iter <- iter + 1
  }

#mean ratio of disparities
mean(disp_test_func_out$props)
